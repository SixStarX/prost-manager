...
export class OiScrapeService {
  ...
  async ingest(payload: ScrapePayload): Promise<ScrapeResult> {
    ...
    if (!kind || !Array.isArray(headers) || !Array.isArray(rows)) {
      throw new BadRequestException(
        'Payload inválido: informe kind, headers e rows.',
      );
    }
    if (rows.length === 0) {
    ...
    switch (kind) {
      case 'orders':
        result = await this.ingestOrders(rows, idx);
        break;
      case 'clients':
        result = await this.ingestClients(rows, idx);
        break;
      case 'vehicles':
        result = await this.ingestVehicles(rows, idx);
        break;
      default:
        throw new BadRequestException(
          `Tipo de coleta desconhecido: ${String(kind)}`,
        );
    }
  ...
  private async ingestClients(
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!name) {
        skipped++;
        errors.push(`Linha ${i + 1}: sem nome de cliente.`);
        continue;
      }
      ...
      try {
        ...
        skipped++;
        errors.push(`Linha ${i + 1} (${name}): ${errorMessage(e)}`);
      }
    }
  ...
  private async ingestVehicles(
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!plate) {
        skipped++;
        errors.push(`Linha ${i + 1}: sem placa.`);
        continue;
      }
      if (!cliName) {
        skipped++;
        errors.push(`Linha ${i + 1} (${plate}): sem cliente/proprietário.`);
        continue;
      }
      ...
      try {
        ...
        if (!client) {
          skipped++;
          errors.push(
            `Linha ${i + 1} (${plate}): cliente "${cliName}" não encontrado. Colete os clientes primeiro.`,
          );
          continue;
        }
        ...
        skipped++;
        errors.push(`Linha ${i + 1} (${plate}): ${errorMessage(e)}`);
      }
    }
  ...
  private async ingestOrders(
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!cliName) {
        skipped++;
        errors.push(`Linha ${i + 1}: OS sem cliente.`);
        continue;
      }
      if (!plate) {
        skipped++;
        errors.push(`Linha ${i + 1} (${cliName}): OS sem placa.`);
        continue;
      }
      ...
      try {
        ...
        if (existing) {
          ...
          const diagnostic = await this.prisma.diagnostic.create({
            data: {
              description:
                desc || 'OS coletada do painel da Oficina Inteligente',
        ...
        skipped++;
        errors.push(`Linha ${i + 1} (${cliName}/${plate}): ${errorMessage(e)}`);
      }
    }
